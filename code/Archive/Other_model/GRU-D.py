...
def create_gru_d_model(input_shape):
    inputs = Input(shape=(input_shape[1], input_shape[2]))  # Adjusted for time series data
...
for file_name in os.listdir(folder_path):
    if file_name.endswith('.csv'):
        ...
        # Prepare the data for GRU-D model (convert to sequences)
        ...
        input_shape = X.shape  # (samples, timesteps, features)
        model = create_gru_d_model(input_shape)
        ...
        imputed_values = model.predict(X).flatten()

        # Add the predicted values back to the DataFrame
        df['PM2.5(original)'] = pm25_col_original

        # Add the imputed PM2.5 values
        df['PM2.5(only_imputed_data)'] = np.nan
        df['PM2.5(only_imputed_data)'][timesteps:] = imputed_values
        ...
        output_file_name = f"{run_number}_{study_site}_{method}_Imputation.csv"
