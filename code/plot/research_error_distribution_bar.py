...
Plot per-model error distributions (bar chart) using per-sample Imputed_Results
...
def plot_bar_errors(err_df: pd.DataFrame, out_path: Path, title=None):
    """
    Create a bar chart summarizing per-model absolute error statistics:
    - bar height = median absolute error
    - error bar = interquartile range (IQR) or std (fallback)
    """
    if err_df.empty:
    ...
    stats = err_df.copy()
    stats["abs_err"] = stats["Error"].abs()
    grouped = stats.groupby("Model")["abs_err"]
    med = grouped.median()
    ...
    summary = pd.DataFrame({
        ...
        "std": std,
        "count": grouped.count()
    }).reset_index().sort_values("median_abs_err")

    if summary.empty:
        logging.error("No per-model summary statistics computed.")
        return False

    # choose errorbar as IQR (preferred) or std if IQR zero
    y = summary["median_abs_err"].values
    yerr = summary["iqr"].values
    zero_iqr = (yerr == 0)
    if zero_iqr.any():
        yerr[zero_iqr] = summary.loc[zero_iqr, "std"].values

    labels = summary["Model"].tolist()

    fig, ax = plt.subplots(figsize=(max(6, 1.2 * len(labels)), 6))
    x = np.arange(len(labels))
    ax.bar(x, y, yerr=yerr, align='center', alpha=0.8, ecolor='black', capsize=6)
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=45, ha="right")
    ax.set_ylabel("Median |Imputed - Actual|")
    if title:
        ax.set_title(title)
    plt.tight_layout()
    ...
    plt.close(fig)
    logging.info("Saved bar error plot: %s", out_path)
    return True
...
def main(argv=None):
    ...
    out_path = Path(args.output_dir) / f"bar_errors_{site}_{args.target.replace('.', '')}.png"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    plot_title = f"Median |Error| per Model — {site} — {args.target}"
    ok = plot_bar_errors(err_df, out_path, title=plot_title)
    if not ok:
        logging.error("Failed to create bar error plot.")
        sys.exit(1)
