...
def load_and_extract_errors(fp: Path, comments_values=("imputed",), require_actual=True):
    ...
    valid = imputed_vals.notna() & actual_vals.notna()
    if not valid.any():
        logging.debug("No numeric imputed+actual pairs in %s after filtering", fp)
        return []

    errors = (imputed_vals[valid] - actual_vals[valid]).to_numpy(dtype=float)
    ...
    missing_val = _first_nonnull(missing_level_col, "")
    ...
    return [{
        ...
        "Missingness_Level": str(missing_val).strip(),
...
def main(argv=None):
    ...
    for fp in csvs:
        ...
        for info in entries:
            ...
            safe_miss = re.sub(r"[^0-9A-Za-z\-_\.]", "_", str(miss))
            safe_target = re.sub(r"[^0-9A-Za-z\-_\.]", "_", target)
            out_fp = Path(args.output_dir) / safe_site / safe_model / safe_regime / f"{safe_miss}_{safe_target}_error_freq.png"
            title = f"{site} | {model} | {regime} | {miss} | {target}"
            ylim_top = site_max.get(site, None)
            color = get_color_for_missingness(miss)
            if args.absolute:
                ok = plot_histogram_absolute(errors, out_fp, bins=args.bins, normalize=args.normalize, title=title, ylim_top=ylim_top, color=color)
            else:
                ok = plot_histogram_signed(errors, out_fp, bins=args.bins, normalize=args.normalize, title=title, ylim_top=ylim_top, color=color)
            if ok:
                total_plots += 1
            else:
                skipped += 1
