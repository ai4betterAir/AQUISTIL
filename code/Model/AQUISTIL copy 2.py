...
def _adaptive_reference(
    ...
    if "adaptive_spatial" in X_missing.columns:
        reference = _safe_numeric(X_missing["adaptive_spatial"]).reindex(missing_index)
    hourly = _hourly_reference(original, target, original_y, observed, missing_index)
    return reference.where(reference.notna(), hourly)
...
def _adaptive_gap_blend_weights(
    ...
    if "gap_long" in rules:
        weights = np.where(gap_length >= 25, np.maximum(weights, float(rules["gap_long"])), weights)
    if "gap_extreme" in rules:
        weights = np.where(gap_length >= 73, np.maximum(weights, float(rules["gap_extreme"])), weights)
...
def _apply_adaptive_gap_guardrails(
    ...
    if not bool(getattr(config, "AQUISTIL_ADAPTIVE_GAP_GUARDRAILS_ENABLED", True)):
        return final_prediction, np.zeros(len(final_prediction), dtype=float)

    weights = _adaptive_gap_blend_weights(target, regime, X_missing, uncertainty, config)
    ...
    weights = np.where(valid_reference, weights, 0.0)
    adjusted = (1.0 - weights) * final_prediction + weights * reference

    clip_targets = getattr(config, "AQUISTIL_ADAPTIVE_CLIP_TARGETS", {}) or {}
    if bool(clip_targets.get(_target_key(target), False)):
        lower, upper = _observed_prediction_bounds(original_y, observed, config)
        adjusted = np.where(weights > 0, np.clip(adjusted, lower, upper), adjusted)
    ...
    logging.info(
        ...
        len(weights),
        float(weights[weights > 0].mean()) if np.any(weights > 0) else 0.0,
    )
