...
def _event_sample_weights(y, train_mask):
...
def impute_mice(data, target_column, input_columns, max_iter=8, tol=1e-4,
    ...
    event_max_blend = float(
        np.clip(
            kwargs.get(
                "event_max_blend",
                getattr(config, "AQUISTIL2_EVENT_MAX_BLEND", 0.65),
            ),
            0.0,
            1.0,
        )
    ...
    for iteration in range(1, max_iter + 1):
        ...
        if use_event_expert:
            event_weights, event_thresholds = _event_sample_weights(y, observed)
        ...
        if event_prediction is not None:
            ...
            event_blend = _event_gate(
                event_prediction, q80, q95, max_blend=event_max_blend
            )
            # The specialist addresses the measured negative event bias only;
            # it cannot pull an ordinary AQUISTIL2 prediction downward.
            event_lift = np.maximum(event_prediction - proposed, 0.0)
            proposed = proposed + event_blend * event_lift
            logging.info(
                "[%s] event expert active_rows=%d/%d mean_lift=%.4f q80=%.3f q95=%.3f",
                MODEL_NAME,
                int((event_blend > 0).sum()),
                len(event_blend),
                float(np.mean(event_blend * event_lift)),
                q80,
                q95,
            )
