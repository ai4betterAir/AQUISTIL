...
def _gap_features(original_missing: np.ndarray, groups: np.ndarray | None = None) -> pd.DataFrame:
...
def _build_features(
    ...
    base = pd.concat([base, _gap_features(original_missing, groups=groups)], axis=1)
...
def impute_mice(
    ...
    **kwargs,
    ...
    logging.info(
        "[%s] ablation switches history=%s spatial=%s event=%s",
        MODEL_NAME,
