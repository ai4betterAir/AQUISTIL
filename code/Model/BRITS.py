...
Simplified BRITS-like imputer compatible with the pipeline.
...
def _prepare_sequence_inputs(df, feature_cols, target_col):
    ...
    Build numpy arrays for model:
      ...
      y: shape (seq_len, 1)
      mask_y: shape (seq_len,) where 1=observed, 0=missing
    """
    ...
    X = X_arr
    y = y_filled[:, np.newaxis]
    mask = mask_y

    return X, y, mask


def _window_sequences(X, y, mask, window_size=168, stride=None):
    """Split one long series into padded fixed-size windows for bounded GRU memory."""
    n_rows = X.shape[0]
    if n_rows == 0:
        return X[:0], y[:0], mask[:0], []

    window_size = int(max(1, window_size))
    stride = int(max(1, stride or window_size))
    starts = list(range(0, n_rows, stride))
    last_start = max(0, n_rows - window_size)
    ...
    for start in starts:
        ...
        length = end - start
        X_win = np.zeros((window_size, X.shape[1]), dtype=np.float32)
        y_win = np.zeros((window_size, 1), dtype=np.float32)
        mask_win = np.zeros((window_size,), dtype=np.float32)
        X_win[:length] = X[start:end]
        y_win[:length] = y[start:end]
        mask_win[:length] = mask[start:end]
        X_windows.append(X_win)
        y_windows.append(y_win)
        mask_windows.append(mask_win)
        metadata.append((start, end, length))

    return (
        np.stack(X_windows).astype(np.float32),
        np.stack(y_windows).astype(np.float32),
        np.stack(mask_windows).astype(np.float32),
        metadata,
    )
...
def impute_mice(
    ...
    **kwargs
    ...
    """
    BRITS-like imputer (pipeline-compatible).
    ...
    try:
        ...
        X_windows, y_windows, mask_windows, window_metadata = _window_sequences(
            X,
            y,
            mask,
        ...
        train_rows = mask_windows.sum(axis=1) > 0
        if not np.any(train_rows):
            logging.warning(f"[{MODEL_NAME}] no trainable windows -> mean fill fallback")
            out = df.copy()
            out[target_column] = out[target_column].fillna(out[target_column].mean())
            return out

        X_train = X_windows[train_rows]
        y_train = y_windows[train_rows]
        sample_weight = mask_windows[train_rows]

        seq_len = X_windows.shape[1]
        n_features = X_windows.shape[2]
        fit_batch_size = max(1, int(batch_size or 16))
        if batch_size == 1:
            fit_batch_size = min(16, len(X_train))
        predict_batch_size = max(1, min(64, fit_batch_size * 4))

        logging.info(
            f"[{MODEL_NAME}] Training TensorFlow GRU on {len(X_train)} windows "
            f"(window_size={seq_len}, batch_size={fit_batch_size})"
        )
        ...
        pred_windows = model.predict(X_windows, batch_size=predict_batch_size, verbose=0)
        pred_sum = np.zeros(X.shape[0], dtype=np.float64)
        pred_count = np.zeros(X.shape[0], dtype=np.float64)
        for i, (start, end, length) in enumerate(window_metadata):
            pred_sum[start:end] += pred_windows[i, :length, 0]
            pred_count[start:end] += 1.0
        pred = (pred_sum / np.maximum(pred_count, 1.0)).astype(np.float32)
