...
def _apply_cli_env_overrides():
    """Allow standalone runs with explicit CLI path flags."""
    ...
    if args.input_dir:
        os.environ["FEATURE_SELECTION_REGION_INPUT_DIR"] = args.input_dir
    if args.output_root:
        os.environ["FEATURE_SELECTION_OUTPUT_ROOT"] = args.output_root
...
# ---------------------------------------------------------------------------
# One-place base directory configuration (user requested)
# ---------------------------------------------------------------------------
DEFAULT_BASE_DIR = APP_ROOT_DIR
BASE_DIR = Path(os.environ.get("IMPUTATION_MODEL_BASE_DIR", str(DEFAULT_BASE_DIR))).resolve()
if not BASE_DIR.exists():
    BASE_DIR = APP_ROOT_DIR

DEFAULT_FEATURE_SELECTION_OUTPUT_DIR = BASE_DIR / "Outputs" / "Feature_Selection" / "04Basics_Stats"
OUTPUTS_DIR = Path(os.environ.get("FEATURE_SELECTION_OUTPUT_ROOT", str(DEFAULT_FEATURE_SELECTION_OUTPUT_DIR))).resolve()
OUTPUTS_DIR.mkdir(parents=True, exist_ok=True)

# For historical naming within this script, keep OUTPUT_DIR as the root output folder.
...
SIMPLE_XGBRF_FEATURES = list(getattr(config, "LOCAL_ANALYSIS_INPUTS", [
...
def candidate_variable_names(target_column="PM2.5"):
    ...
    variables.update(["TEMP", "HUMID", "WSP", "WDR", "RAIN", "WGU", "NEPH"])
...
def cleanup_non_basics_outputs():
    """
    Remove non-Basics artifacts so this script leaves only Basics plots behind.

    Keeps:
    - OUTPUT_DIR / "Basics"
    - feature_selection_analysis.log
    """
    keep_dirs = {BASICS_DIR.resolve()}
    keep_files = {LOG_FILE.resolve()}
    removable_suffixes = {".png", ".csv", ".json", ".txt", ".pdf"}

    # Remove known non-Basics directories wholesale.
    for candidate in [REGIONAL_ASSESSMENT_DIR]:
        try:
            if candidate.exists():
                shutil.rmtree(candidate)
                logging.info(f"Removed non-Basics directory: {candidate}")
        except Exception:
            logging.exception(f"Failed removing non-Basics directory: {candidate}")

    # Remove loose output artifacts in OUTPUT_DIR except the log file.
    for entry in OUTPUT_DIR.iterdir():
        try:
            resolved = entry.resolve()
            if resolved in keep_dirs or resolved in keep_files:
                continue
            if entry.is_file() and entry.suffix.lower() in removable_suffixes:
                entry.unlink()
                logging.info(f"Removed non-Basics file: {entry}")
            elif entry.is_dir() and resolved not in keep_dirs:
                shutil.rmtree(entry)
                logging.info(f"Removed non-Basics directory: {entry}")
        except Exception:
            logging.exception(f"Failed cleaning output artifact: {entry}")

    # Remove extra JSON written outside OUTPUT_DIR.
    extra_json = BASE_DIR / "BestPredictors_ByRegionTarget.json"
    try:
        if extra_json.exists():
            extra_json.unlink()
            logging.info(f"Removed non-Basics file: {extra_json}")
    except Exception:
        logging.exception(f"Failed removing non-Basics file: {extra_json}")
...
def write_region_target_best_predictors_json(
    ...
    """
    Save a JSON summary (no performance metrics):
      { "<region>": { "<target>": { "best_predictors": [...]} } }

    best_predictors are derived from:
    ...
    # Normalize selected regions so "Central Coast" matches "Central_Coast" etc.
    region_tokens = {normalize_region_token(r) for r in (selected_regions or []) if str(r).strip()}

    # Overall best single feature for this target (compiled scope) - store only the feature name
    try:
    ...
    for region_name in sorted(single_feature_results_df["region"].unique()):
        if region_name == "ALL_SELECTED":
            continue
        if region_tokens and normalize_region_token(region_name) not in region_tokens:
            continue
        ...
        # Provide both top-4 and top-5 convenience lists, plus a selected default.
        target_entry["best_predictors_top4"] = best_predictors[:4]
        target_entry["best_predictors_top5"] = best_predictors[:5]
        target_entry["best_predictors"] = target_entry["best_predictors_top5"] if int(top_n_single) >= 5 else best_predictors[: int(top_n_single)]
...
def write_best_predictors_by_region_target_json(
    ...
    """
    Write a single JSON:
      { "<region>": { "<target>": ["feat1", "feat2", ...] } }

    Uses per-region single-feature ranking (lowest RMSE) from the multi-target results table.
    """
    # Save the single JSON at the base directory (user requested)
    #   <BASE_DIR>/BestPredictors_ByRegionTarget.json
    output_dir = Path(output_dir)
    payload = {}

    if multi_target_results_df is None or multi_target_results_df.empty:
        raise ValueError("multi_target_results_df is empty")

    required_cols = {"target_column", "scope", "region", "feature", "rmse"}
    missing = required_cols - set(multi_target_results_df.columns)
    if missing:
        raise ValueError(f"Missing required columns for summary JSON: {sorted(missing)}")

    per = multi_target_results_df[multi_target_results_df["scope"] == "per_region"].copy()
    if per.empty:
        raise ValueError("No per_region rows found for summary JSON")

    for (region_name, target_name), group in per.groupby(["region", "target_column"]):
        group = group.sort_values("rmse")
        best_feats = group["feature"].astype(str).head(int(top_n)).tolist()
        payload.setdefault(str(region_name), {})[str(target_name)] = best_feats

    out_path = BASE_DIR / "BestPredictors_ByRegionTarget.json"
    with open(out_path, "w") as f:
        json.dump(payload, f, indent=2)
    logging.info(f"Best predictors by region/target JSON saved: {out_path}")
    return out_path
...
def make_xgbrf_model(seed=42):
    """Compatibility helper now returning the shared RF selection model."""
    return Pipeline([
...
def run_simple_xgbrf_feature_selection(
    ...
    """Generate only Basics diagnostics for the selected regional inputs."""
    selected_regions = selected_regions if selected_regions is not None else []
    ...
    cleanup_non_basics_outputs()
    logging.info(f"Generating Basics plots using {len(files)} regional CSV file(s) for target={target_column}")
    ...
    logging.info("Basics-only mode complete for target=%s", target_column)
    return pd.DataFrame()
...
def run_simple_xgbrf_multi_target_heatmaps(
    ...
    """Generate only Basics diagnostics for each requested target."""
    cleanup_non_basics_outputs()
    for target_column in target_columns:
        logging.info("=" * 80)
        logging.info(f"Running Basics-only regional plotting for target: {target_column}")
        run_simple_xgbrf_feature_selection(
            target_column=target_column,
    ...
    logging.info("Basics-only mode complete. No non-Basics outputs were written.")
    return pd.DataFrame()
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["simple_rf"],
        default="simple_rf",
        help="simple_rf compares compiled vs per-region Random Forest RMSE.",
    )
    parser.add_argument(
        "--target",
    ...
    parser.add_argument(
        "--targets",
        default=SELECTED_TARGET,
        help="Comma-separated target variables for simple_rf heatmaps.",
    )
    parser.add_argument(
        "--regions",
        default=",".join(SELECTED_REGIONS),
        help="Comma-separated region names. Default uses all available regional processed CSV files.",
    )
    parser.add_argument(
        "--input-dir",
        default=str(REGIONAL_INPUT_DIR),
        help="Wide regional input folder containing Allobs_processed_DPE_station_api_*_ALL.csv.",
    )
    parser.add_argument(
        "--output-root",
        default=str(OUTPUT_DIR),
        help="Root output folder for this standalone basics run.",
    )
    parser.add_argument(
        "--missing-frac",
        type=float,
    ...
    if args.mode == "simple_rf":
        ...
        logging.info(
            f"Running simple Random-Forest-only heatmaps for targets={', '.join(target_columns)}; "
...
if __name__ == "__main__":
    main()
    print("=== JOB FINISHED: Stage 4 basic feature statistics ===", flush=True)
