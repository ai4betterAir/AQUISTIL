#!/usr/bin/env python3
"""Create all AQUISTIL residual plots in the scatter-plot style.
...
excluded to match ``scatter_all_scopes.py``.
...
def parse_args():
    ...
    parser.add_argument("--x-axis-percentile", type=float, default=99.9)
...
def load_target(results_root, model, target):
    ...
    data["Residual"] = data["Imputed"] - data["Observed"]
    return data, files


def metrics(part):
    observed = part["Observed"].to_numpy(dtype=float)
    residual = part["Residual"].to_numpy(dtype=float)
    if len(observed) == 0:
...
def draw_panel(ax, part, colour, background, x_upper, y_abs, rng, max_points):
    bias, mae, rmse = metrics(part)
    shown = part
    ...
    ax.axhline(0, color=CHARCOAL, linestyle="--", linewidth=0.95)
    ax.text(
        0.04,
        0.96,
        "Bias={:.2f}\nMAE={:.2f}\nRMSE={:.2f}\nn={:,}".format(bias, mae, rmse, len(part)),
...
def render_scope(data, target, model, scope_label, output_dir, args):
    ...
    x_upper = float(np.percentile(data["Observed"].to_numpy(dtype=float), args.x_axis_percentile))
    x_upper += max(x_upper * 0.04, 0.5)
    residual_abs = np.abs(data["Residual"].to_numpy(dtype=float))
    y_abs = float(np.percentile(residual_abs, args.residual_percentile))
    y_abs = max(y_abs + max(y_abs * 0.10, 0.5), 1.0)
    ...
    for regime in REGIMES:
        ...
        output = output_dir / "regime_{}_six_missingness_A4_residual.png".format(regime)
        ...
        for index, level in enumerate(levels):
            ...
            draw_panel(ax, part, colour, background, x_upper, y_abs, rng, args.max_points_per_panel)
            ax.text(
                0.50,
                0.96,
                "{:g}%".format(level * 100),
        ...
        fig.text(
            0.006,
            0.5,
            "Residual: Imputed - Observed {}".format(pollutant),
            va="center",
            rotation="vertical",
            fontsize=20.4,
        )
        fig.suptitle(
            "{}: {} Residuals — {}".format(model, regime_label, scope_label),
            fontsize=21.6,
            fontweight="bold",
            y=0.965,
        )
        fig.subplots_adjust(left=0.075, right=0.99, bottom=0.085, top=0.90, wspace=0.025, hspace=0.10)
...
def main():
    args = parse_args()
    if not 90 <= args.x_axis_percentile <= 100:
        raise ValueError("--x-axis-percentile must be between 90 and 100")
    if not 90 <= args.residual_percentile <= 100:
        raise ValueError("--residual-percentile must be between 90 and 100")
    targets = args.targets or discover_targets(args.results_root, args.model)
    plot_root = args.results_root / "plots_by_type" / "residual_plot"
    total = 0
